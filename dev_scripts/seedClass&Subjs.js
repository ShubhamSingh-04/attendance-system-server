...
const seedDatabase = async () => {
  try {
    ...
    console.log('Class documents created.'); // <-- Added this log line
    ...
    const allSubjects = [
      ...sem5SubjectsData.map((sub) => ({
        ...sub,
        class: class5._id,
        // semester: 5, <-- REMOVED: This field doesn't exist on Subject
      })),
      ...sem6SubjectsData.map((sub) => ({
        ...sub,
        class: class6._id,
        // semester: 6, <-- REMOVED: This field doesn't exist on Subject
      })),
      ...sem7SubjectsData.map((sub) => ({
        ...sub,
        class: class7._id,
        // semester: 7, <-- REMOVED: This field doesn't exist on Subject
      })),
    ];
